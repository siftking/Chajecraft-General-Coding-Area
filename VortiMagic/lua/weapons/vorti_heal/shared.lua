...
SWEP.HealDelay				= 3.0	
SWEP.HealthDelay			= 5.0	
SWEP.MaxArmor				= 18	
SWEP.MinArmor				= 12
SWEP.MinHealth				= 20
SWEP.MaxHealth				= 20
SWEP.HealthLimit			= 150
SWEP.ArmorLimit				= 50	
SWEP.BeamHeal				= 45
SWEP.BeamChargeTime			= 1.0
SWEP.Deny					= Sound("Buttons.snd19")			

SWEP.Primary.ClipSize		= -1
SWEP.Primary.AmmoPerUse     = 2
SWEP.Primary.DefaultClip	= 20
SWEP.Primary.Ammo 			= "ammo_vortenergy"
...
function SWEP:Shoot(dmg,effect)
	...
	if SERVER then
		...
		DMG:SetDamage(dmg or self.BeamHeal)
...
function SWEP:Think()
	...
	if self.Charging && self.ChargeTime-0.25 < CurTime() && !self.attack then
		...
		if self.Owner:GetAmmoCount(self.Primary.Ammo)>=self.Primary.AmmoPerUse then//check always if we have ammo
			...
			//timer.Simple(0.7,function()if !IsValid(self.Owner) || self.Owner:GetActiveWeapon()!=self || !IsValid(self)  then return end self.Weapon:SendWeaponAnim(ACT_VM_IDLE)end)
		end
		self.attack=true;
	end
	...
	if self.Healing && self.HealTime<CurTime() then
		if self.Owner:GetAmmoCount(self.Primary.Ammo)<self.Secondary.AmmoPerUse || self.Owner:Health()>=self.HealthLimit then 
		self.Weapon:EmitSound(self.Deny)
		self.Weapon:SetNextPrimaryFire(CurTime()+SoundDuration(self.Deny))
		self.Weapon:SetNextSecondaryFire(CurTime()+SoundDuration(self.Deny))
		if IsValid(self.Owner:GetViewModel())then self.Owner:GetViewModel():StopParticles() end
		...
		self.Owner:RemoveAmmo(self.Secondary.AmmoPerUse,self.Primary.Ammo);
		if IsValid(self.Owner:GetViewModel())then self.Owner:GetViewModel():StopParticles() end
		self.Owner:StopParticles()
		self.Weapon:SendWeaponAnim(ACT_VM_IDLE)
		self.Healing=false;
...
function SWEP:GiveHealth()
	...
	if plarm>=self.HealthLimit then return end
	if plarm <= (self.HealthLimit - arm) then
	self.Owner:SetHealth( plarm + arm )
	else
	self.Owner:SetHealth(self.HealthLimit)
	end
end
...
function SWEP:SecondaryAttack()
	if self.Charging || self.Healing || self.Owner:Health()>=self.HealthLimit then return end
		if self.Owner:GetAmmoCount(self.Primary.Ammo)<self.Secondary.AmmoPerUse  then 
		self.Weapon:EmitSound(self.Deny)
		self.Weapon:SetNextPrimaryFire(CurTime()+SoundDuration(self.Deny))
		self.Weapon:SetNextSecondaryFire(CurTime()+SoundDuration(self.Deny))
		return 

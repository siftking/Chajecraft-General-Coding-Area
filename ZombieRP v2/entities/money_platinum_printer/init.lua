...
local minPrint 			= 140							-- 	The minimum amount the printer can generate.
local maxPrint			= 160							-- 	The maximum amount the printer can generate.
local upgradedExtra 	= maxPrint * 0.5				-- 	If the printer is upgraded, how much extra will they get?
local printerColor 		= Color( 200, 200, 225, 255 )	--  The color of the actual printer prop.
...
function ENT:Use(activator)
	if ( activator:IsPlayer() and self:GetNWInt("PrintA") >= 1 ) then
		...
		DarkRP.notify(activator, 0, 4, "You have collected $" .. self:GetNWInt("PrintA").. " from a Coal Printer." )
		self:SetNWInt("PrintA",0)
	end
end
